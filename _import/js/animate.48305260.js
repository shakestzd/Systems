...
export function mountScrollChart(chartEl, updateFn, steps, opts = {}) {
  ...
  const above = opts.callout === "above";
  ...
  const onScroll = () => {
    ...
    // Steps start when section top hits viewport midpoint (50%)
    // and must ALL complete while chart is still 5% from viewport top.
    // Guarantees every step fires while the chart is fully visible.
    const startY = vh * 0.50;
    const endY   = vh * 0.05;
