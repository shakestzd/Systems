...
import { INK, INK_LIGHT, RULE, chartW, isMobile as _isMobile } from "../design.2d3db129.js";
...
export function createDemandThesis(stats) {
  ...
  const srcX = 36;
  const srcR = 42;
  const mobileCardX = srcX + srcR + 16;   // = 94px from left
  const CW       = isMobile ? W - mobileCardX - 10 : 180;
  const CW_EXP   = isMobile ? CW : 300;   // mobile cards already full-width, no expand
  ...
  // Column x positions (desktop only); stagger by COL_DX for second card in each group
  const COL_X  = [88,  316, 538];
  const COL_DX = [22,  22,  22];

  // Y positions — derived so no two cards ever overlap vertically (same for both layouts)
  const baseY = 48;
  const ys = (() => {
    const r = [];
  ...
  // Mobile: single column (all cards stacked vertically, right of source circle)
  // Desktop: three columns with stagger
  const positions = isMobile
    ? theses.map((_, i) => ({ x: mobileCardX, y: ys[i] }))
    : [
        { x: COL_X[0],              y: ys[0] },
  ...
  // ── X axis: time horizon labels at top (desktop only — on mobile the zone
  //    group labels on each card carry this information) ───────────────────
  if (!isMobile) {
    [
  ...
  // Desktop: horizontal axis labels at opposite ends of the card area.
  // Mobile: single-column layout makes left/right arrows meaningless — omit.
