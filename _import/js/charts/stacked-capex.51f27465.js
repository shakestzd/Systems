...
import { INK, INK_LIGHT, ACCENT, RULE, svgTitle, svgStepAnnot, svgSource, chartW, isMobile as _isMobile } from "../design.2d3db129.js";
import { cl } from "../design.2d3db129.js";
...
export function createStackedCapex(capex, stats) {
  ...
  // Legend — 2 rows on mobile to prevent overflow
  if (isMobile) {
    [[2022, 2023, 2024], [2025, 2026]].forEach((row, ri) => {
      row.forEach((yr, i) => {
        const lx = ml + i * 72;
        const ly = H - 50 + ri * 18;
        svg.append("rect").attr("x", lx).attr("y", ly)
          .attr("width", 10).attr("height", 10)
          .attr("fill", yearColors[yr]).attr("rx", 1)
          .attr("opacity", yr === 2026 ? 0.8 : 1);
        svg.append("text").attr("x", lx + 13).attr("y", ly + 9)
  ...
  const STEP_ANNOTS = [
    "Six companies, every year from 2022 \u2014 hatched bar shows 2026 guidance",
    "Amazon has spent more than any other company since 2022",
